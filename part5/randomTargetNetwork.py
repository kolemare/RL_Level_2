...
# Define a target model (for the target Q-network)
...
# Copy the weights of the main model to the target model initially
...
# Synchronization frequency for the target network
sync_freq = 500  # Update target network every 500 steps
j = 0  # Counter for steps
...
for i in range(epochs):
    ...
    while status == 1:
        ...
        # Forward pass to get Q-values for the current state from the main model
        qval = model(state1)
        ...
        # Synchronize target network with main network at specified intervals
        if j % sync_freq == 0:
            model2.load_state_dict(model.state_dict())
...
plt.savefig("randomTargetNetwork.png")
